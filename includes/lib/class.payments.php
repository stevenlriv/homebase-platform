...
if ( !defined('SCRIP_LOAD') ) { die ( header('Location: /not-found') ); }
	 ...
	 */
	function get_payments ($type, $query = '', $extra = '') {
		global $db;
		...
		if(is_array($query)) {
			...
			$q = $db->prepare ( "SELECT * FROM xvls_users_payments $build_query $extra" );
			...
			$q = $db->prepare ( "SELECT * FROM xvls_users_payments WHERE id_payment = ? LIMIT 1" );
