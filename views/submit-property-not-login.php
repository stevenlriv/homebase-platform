<?php
	if ( !defined('THEME_LOAD') ) { die ( header('Location: /not-found') ); }
?>
<!-- Titlebar
================================================== -->

<!-- Fullwidth Section -->
<section class="fullwidth margin-top-105" data-background-color="#ffffff">

	<!-- Box Headline -->
	<h3 class="headline-box">What role describes you the best?</h3>
	
	<!-- Content -->
	<div class="container">
		<div class="row">

			<div class="col-md-6 col-sm-12">
				<!-- Icon Box -->
				<div class="icon-box-1">

					<div class="icon-container">
						<i class="im im-icon-Geek"></i>
						<div class="icon-links">
							<a href="/for-realtors">Learn More</a>
						</div>
					</div>

					<h3>Realtor</h3>
					<p>Have a couple of clients who might like to be in our platform.</p>
				</div>
			</div>

			<div class="col-md-6 col-sm-12">
				<!-- Icon Box -->
				<div class="icon-box-1">

					<div class="icon-container">
						<i class="im im-icon-Cap-Smiley"></i>
						<div class="icon-links">
							<a href="/for-landlords">Learn More</a>
						</div>
					</div>

					<h3>Landlord</h3>
					<p>Own 1 or more properties. Looking for help and automation.</p>
				</div>
			</div>

		</div>
	</div>
</section>
<!-- Fullwidth Section / End -->

<!-- Flip banner -->
<a href="/find-a-homebase" class="flip-banner parallax" data-background="/views/assets/images/intro-minimized.jpg" data-color="#274abb" data-color-opacity="0.9" data-img-width="2500" data-img-height="1600">
	<div class="flip-banner-content">
		<h2 class="flip-visible">We help people and homes find each other</h2>
		<h2 class="flip-hidden">Browse Properties <i class="sl sl-icon-arrow-right"></i></h2>
	</div>
</a>
<!-- Flip banner / End -->




